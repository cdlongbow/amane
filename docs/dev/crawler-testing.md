...
用例在独立仓库 `amane-testdata`; `just fixtures` 按 `tests/.fixtures-rev` 钉住的 sha 检出到 `tests/crawlers/cases/` (gitignored). `just test` 与 `just test-cov` 会先执行该依赖, `just check` 通过 `test-cov` 间接执行, 不单独声明. CI 经 `FIXTURES_DEPLOY_KEY` 经由 SSH.
...
每个 TOML **顶部注释必须记录**采集命令 (curl / `uv run python -c` / 浏览器步骤), 方便刷新. 一个 TOML 一个场景, 不能放入多个 case.
