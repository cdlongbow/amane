...
> 入口: `src/amane/api/routes/`. 契约在 `api/models/` (与 routes 对齐). 端点签名从源码或 `web/openapi.json` 读 (`just generate`); 本文只写划分、约定与易回归点.
...
端点经由 `Depends` 从 `app.state.runtime` 读取. `deps.py` 预声明 `RuntimeDep` / `RepoDep` / `ConfigDep` / `AgentDep` (`agent_service` 未接入 → 503).
