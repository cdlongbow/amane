...
- `number_pattern` 一旦设置, **只**走该正则 (title → description → link; 有捕获组用 group 1), 不回退 `extract_number`. 未设置才用 `extract_number` (命中已知形态则按 `parse_file_info` 重写, **无**文件名「清理后原样」回退). 重写与爬虫入参契约见 [crawlers.md](crawlers.md).
