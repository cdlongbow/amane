...
r18 库**不能纳入 Alembic** — 项目对其只读, schema 随 r18 dump 变化; 兼容性靠导入期探针校验, 不靠迁移.
...
- 文件级备份方便 (见下方迁移前自动备份; 手工备份须用 online backup. WAL 模式下不能只 `cp amane.db`).
...
CLI (`uv run alembic …`) 使用同一套 `env.py` 事务性 DDL; CLI **不会**自动备份 — 依赖启动路径或手工 backup. `env.py` 自行创建的引擎在 upgrade 结束后必须 `dispose()`, 否则同一进程内多次升级 (测试) 会留下未关闭的 sqlite3 连接. 迁移脚本导入约定见 [architecture.md](architecture.md).
...
用户更新版本后**不需要手动迁移** — Docker 拉取新镜像并重启即可. 启动失败时:

- `AMANE_DATA_DIR` 指向独立目录隔离
- 用最近的 `*.pre-migrate-*.bak` 恢复 (先停服务, 换回主库文件并删除 `-wal`/`-shm`)
...
- **JSON 列内部结构** — `Metadata.raw` 等 blob 不在 autogenerate 视野里. 爬虫/聚合模型修改字段名或类型时, **结果列与 raw 快照是两份数据**; 只修改列定义不够, 必须另写 data revision 遍历 JSON (见 `c4f17334c3ea`). 站点级复用会把 raw 直接 `MediaMetadata(...)`, 旧 key 会被 Pydantic 静默丢掉.
...
- **path 投影列** — `MediaFile.content_type` / `mosaic` 与 `status` 同为枚举列; `definition` 不是枚举. 忽略 `ix_metadata_number` 表达式索引噪声.
...
2. 用 `op.get_bind()` 执行裸 SQL **回填** (basename、最长前缀匹配归属等)
...
`alembic.ini` 中 `sqlalchemy.url` 默认 `sqlite:///data/amane.db`, 与 `AMANE_DATA_DIR=./data` 对齐. 修改 `data_dir` 后手动执行 alembic 命令需:
